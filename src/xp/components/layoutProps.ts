// Layout style-prop helpers. Output uses camelCase keys / numeric `px` values
// (React-style); pass the result through `s()` (src/common/style.ts) before
// handing it to a Solid `style` prop.
...
const applySpacing = (style: StyleObject, props: SpacingProps) => {
  const {
    gap,
    margin,
    marginX,
  ...
  if (resolvedMarginRight !== undefined) style.marginRight = resolvedMarginRight;
  ...
  if (resolvedPaddingLeft !== undefined) style.paddingLeft = resolvedPaddingLeft;
};

const applySizing = (style: StyleObject, props: SizingProps) => {
...
const applyDecoration = (style: StyleObject, props: DecorationProps) => {
...
export const createBaseStyle = (props: BaseStyleProps): StyleObject => {
...
export const mapAlignToCss = (align?: Align): string | undefined => {
...
export const mapJustifyToCss = (justify?: Justify): string | undefined => {
